...
export const sendMessage = async (data: any, callback?: Function) => {
    ...
    try {
        ...
        if (!room) {
            ...
            let senderData = await User.findById(sender._id).select("socketId");
            let reciverData = await User.findById(reciver).select("socketId");
            ...
            if (reciverData?.socketId) {
                ...
                if (newRoomForReciver) io.to(reciverData.socketId).emit("receiveRoom", newRoomForReciver)
            }

            io.to(senderData?.socketId || "").to(reciverData?.socketId || "").emit("receiveMessage", {
...
export const getMessage = async (req: Request, res: Response) => {
    ...
    try {
        ...
        const oldRoom = await ChatRoom.findOne({ participants: { $all: [receiver, sender.id] } }).populate({
            path: "messages",
            populate: {
                path: "sender",
                select: "name _id"
        ...
        res.status(200).json({ message: "message Fetched sucessfully", allMessages: oldRoom?.messages || [] });
    } catch (error) {
        console.log(error);
...
export const getContacts = async (req: Request, res: Response) => {
    ...
    try {
        ...
        const sender: jwtTokenType | null = (jwt.decode(token) as jwtTokenType | null);
        ...
        res.status(200).json({ message: "message Fetched sucessfully", rooms: rooms || [] });
    } catch (error) {
        console.log(error);
        res.status(500).json({ message: "Error while Reciving message" });
    }
...
export const searchContact = async (req: Request, res: Response) => {
    ...
    try {
        ...
        res.status(200).json({ message: "Contacts Fetched sucessfully", users: user || [] });
    } catch (error) {
        console.log(error);
